...
public class MainActivity extends Activity {
    ...
    private SeekBar promptWidth;
    private SeekBar promptPosition;
    private SeekBar readingLine;
    ...
    private View buildUi() {
        ...
        TextView subtitle = text("Native Overlay Prompter — v0.1", 14, MUTED);
        subtitle.setPadding(0, dp(2), 0, dp(16));
        root.addView(subtitle);

        TextView explainer = text(
                "This version does not replace the Pixel camera. It places the teleprompter over the native Pixel Camera app, so Google Camera keeps control of the camera, microphone, stabilization, bitrate, and processing.",
                14, TEXT);
        ...
        root.addView(sectionTitle("TELEPROMPTER"));
        speed = addSeek(root, "Scroll speed", 8, 90, 32, "");
        textSize = addSeek(root, "Text size", 22, 60, 34, " sp");
        promptWidth = addSeek(root, "Prompt width", 42, 96, 76, "%");
        promptPosition = addSeek(root, "Prompt position", 4, 75, 18, "% from bottom");
        readingLine = addSeek(root, "Reading line", 20, 80, 50, "% inside prompt");
        ...
        TextView note = text(
                "The overlay is a separate Android window. Pixel Camera records the camera stream, not the screen, so the teleprompter text is not intended to appear in the saved video. This app requests no camera or microphone permission.",
                12, MUTED);
    ...
    private void loadSettings() {
        ...
        setSeek(speed, prefs.getInt("speed", 32));
        setSeek(textSize, prefs.getInt("textSize", 34));
        setSeek(promptWidth, prefs.getInt("promptWidth", 76));
        setSeek(promptPosition, prefs.getInt("promptPosition", 18));
        setSeek(readingLine, prefs.getInt("readingLine", 50));
    ...
    private void saveSettings() {
        prefs.edit()
                ...
                .putInt("speed", seekValue(speed, 8))
                .putInt("textSize", seekValue(textSize, 22))
                .putInt("promptWidth", seekValue(promptWidth, 42))
                .putInt("promptPosition", seekValue(promptPosition, 4))
                .putInt("readingLine", seekValue(readingLine, 20))
    ...
    private SeekBar addSeek(LinearLayout root, String name, int min, int max, int initial, String suffix) {
        LinearLayout block = new LinearLayout(this);
        block.setOrientation(LinearLayout.VERTICAL);
        block.setPadding(0, dp(7), 0, dp(7));

        TextView out = label(String.format(Locale.US, "%s  %d%s", name, initial, suffix));
        block.addView(out);
        ...
        seek.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            @Override public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                out.setText(String.format(Locale.US, "%s  %d%s", name, progress + min, suffix));
            }
    ...
    private int seekValue(SeekBar bar, int min) { return bar.getProgress() + min; }
    private void setSeek(SeekBar bar, int value) {
        int min = (int) bar.getTag();
        bar.setProgress(Math.max(0, value - min));
    }
